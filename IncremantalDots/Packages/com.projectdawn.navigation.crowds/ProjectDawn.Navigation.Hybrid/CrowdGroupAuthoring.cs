...
namespace ProjectDawn.Navigation.Hybrid
{
    ...
    public class CrowdGroupAuthoring : EntityBehaviour
    {
        ...
        public void SetGoals(float3 min, float3 max)
        {
            ...
            var group = world.EntityManager.GetComponentData<CrowdGroupFlow>(m_Entity);
            group.Flow.ClearGoals();
            group.Flow.AddGoal(min, max);
        }

        /// <summary>
        /// Changes the surface. This is expensive operation.
        /// </summary>
        public void SetSurface(CrowdSurfaceAuthoring surface)
        {
            m_Surface = surface;
            ...
            group.Surface = GetComponent<CrowdSurfaceAuthoring>().GetOrCreateEntity();
            EntityGroup = group;

            var entityManager = World.DefaultGameObjectInjectionWorld.EntityManager;
            if (entityManager.HasComponent<CrowdGroupFlow>(m_Entity))
                entityManager.RemoveComponent<CrowdGroupFlow>(m_Entity);
        }
