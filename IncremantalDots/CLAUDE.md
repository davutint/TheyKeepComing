...
| Ana menu SAHNESI (M-E v2) | `MainMenuSceneUI.cs` + `GameBootstrap.cs` + `RunBootstrap.cs` | AYRI `MainMenuScene.unity` (build 0): runtime-uretilen gorseller (gradyan+kanli ay+rounded butonlar, MenuSpriteFactory) + DOTween giris; CONTINUE — DAY X; secim GameBootstrap.PendingAction -> oyun sahnesinde RunBootstrap uygular |
| Pause + ayarlar | `PauseMenuUI.cs` + `SettingsUI.cs` + `SoundSettings.cs` | sag-ust II butonu (RESUME/SETTINGS/NEW RUN/MAIN MENU); SFX/ambiyans slider'lari (PlayerPrefs); settings paneli iki sahnede ortak ureticiyle |
| Safak-checkpoint save | `RunPersistence.cs` + GameManager Save/Restore | her DAWN otomatik kayit (run_save.json); olum/NEW RUN siler; CONTINUE son safaktan yukler (yalniz recompute-EDILEMEYEN kaydedilir) — dok: RUN_SAVE_ARCHITECTURE.md |
