...
- Wall current HP, Basic/Rapid/Frost sayıları, `ArcherFormationVersion`, exact Heart graph level state'i, tech node level'ları ve dormant legacy level-up tier'ları. `ArcherTypeLevels` yalnız v3-v16 JSON migration girdisidir; v17 save'de boş normalize edilir.
...
1. Geçerli `v3`-`v17` snapshot yüklenir; legacy state açık migration zinciriyle in-memory v17'ye yükseltilir. Saved Heart graph, v14 combat rebuild payload'ı ve v15 run telemetry sırası temiz runtime kurulmadan önce validate edilir.
...
4. `ArcherFormationVersion` yüklenir; mevcut başlangıç okçuları aynı formation cache'ine taşınır, ardından archer type count state'i, kaynaklar, finite Arrow paid state'i ve Grave Essence bakiyesi geri yazılır. Direct type-level state restore edilmez. Exact Heart graph effect'leri deferred pipeline ile replay edilir; existing/yeni okcular aynı effective stat'i alır ve Arrow effective capacity son aggregate sonrasında bir kez clamp edilir.
...
- `RunPersistenceTests.TryLoad_Version16Snapshot_RetiresDirectArcherLevelsWithoutInventingHeartState`
- `RunPersistenceTests.TryLoad_Version17InvalidCombatRebuild_FailsClosed`
- `RunPersistenceTests.TryLoad_Version17OutOfOrderRunTelemetry_FailsClosed`
...
- `HeartGraphContinuePlayModeTests.HeartArcherEffects_RebaseExistingAndFutureUnits_AndContinueDoesNotCompound`
