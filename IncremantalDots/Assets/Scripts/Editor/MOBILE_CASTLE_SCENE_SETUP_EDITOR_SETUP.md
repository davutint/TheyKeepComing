...
- `FirstRunOnboardingUI`: scene HUD root'ta tek component; `WorkerEconomyDrawerUI`, `MarketUI`, `ArrowSupplyUI`, `HeartScreenUI`, `CouncilEventUI`, `DefenseRepairUI`, `SpellCastUI`, `OnboardingHintPanel/Text` ve `OnboardingPulseFrame` referanslari dolu
...
- Beklenen onboarding presentation: `OnboardingHintPanel`, `OnboardingHintText`, `OnboardingHintAccent`, `OnboardingPulseFrame`; prefabda inactive/raycast kapali, runtime scene owner'i tarafindan acilir. First Night ability-key cue'su bottom-center `0,170` konumunda ilk hazir `FireballButton`, `RallyAbilityButton` veya `EmergencyRepairAbilityButton` slotunu hedefler
...
- `Window > DeadWalls > Repair First Run Onboarding`, paylasilan presentation'i idempotent kurar ve aktif sahnedeki tek controller binding'ini onarir.
- `Window > DeadWalls > Repair Narrative Opening Copy`, `MainMenuScene` title/tagline/new-stand metnini `MainMenuSceneUI` canon sabitlerinden idempotent uygular; uzun prologue modal'i eklemez.
- `Window > DeadWalls > Repair Tutorial Reset Setting`, `NewGameScene` aktifken hem pause Settings hem `MainMenuScene` Settings icin iki-tik confirmation button/status isim sozlesmesini idempotent kurar ve iki sahneyi kaydeder.
...
- Beklenen continuous cycle alanlari: `CyclePanel`, `CycleDayCounterText`, `CycleProgressTrack`/`CycleCelestialArc`, `CycleProgressMarker`, `CycleCelestialGlow`; legacy phase/label/fill objeleri inactive uyumluluk katmanidir
