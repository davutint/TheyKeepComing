...
- [x] Day/night lighting ve combat feedback owner görsel review'dan geçiyor; kapsamlı audio asset polish'i owner kararıyla bu V1 kabul kapısının dışında.
  - Kanıt: Owner, Day/Night geçişlerini ve yoğun sürü sunumunu açıkça kabul etti. İlk yön düzeltmesinin equal-progress deadlock'u kaldırmasına rağmen Fireball çemberinin teğetindeki hedefe az miktarda yakın çapraz `Queued` komşular sahte blocker zinciri kurabiliyordu. `ZombieQueueFlowUtility.CanBlockQueue`, blocker'ı hem hedefe yakınlık hem `45 derece` ileri hareket koridoruyla sınırlar; doğrudan kuyruk korunurken çember teğeti açılır. Exact gündüz 10K koşusunda darbe öncesi hedef/çevre yoğunluk oranı `%101`; `r=3,5` strike `994` zombiyi öldürdükten `5,5s` sonra oran `%99` ve görünür dairesel boşluk yoktur. Utility EditMode `7/7`, yan-komşu + yeni çapraz-yay PlayMode `2/2`; final Console `0 error` ve Game View QA temiz.
...
| 2026-07-16 | `DW-I-QUEUE-CAVITY-FOLLOWUP` forward-lane horde refill after circular strikes | Önceki yalnız progress tabanlı düzeltmenin eksik kalan çapraz-teğet vakası kapatıldı. `ZombieQueueFlowUtility.CanBlockQueue`, single-front/legacy için `-X`, radial için kale merkezine yönelen eksen üzerinde ileri mesafeyi ve lateral mesafeyi karşılaştırır; yalnız `45 derece` ileri koridordaki Attacking/Queued komşu blocker olur. Fireball damage/radius/cooldown, Day/Night fazları, spawn yoğunlukları, temel hız ve doğrudan kuyruk davranışı değişmedi. Yeni çapraz-yay PlayMode regresyonu üç Queued zombinin açılan koridora dönmesini kilitler | Unity compile `0 error`; utility EditMode `7/7`; mevcut yan-komşu + yeni çapraz-yay PlayMode `2/2`; exact Day 10K canlı koşuda `994` ölüm sonrası `5,5s` hedef/çevre yoğunluğu `%99`, görünür çember yok; final Console `0 error`. Tracker kapsamı değişmedi: `392/442` |
