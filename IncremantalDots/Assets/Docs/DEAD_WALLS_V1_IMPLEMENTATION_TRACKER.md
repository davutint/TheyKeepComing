...
> **Aktif iş:** `DW-I-ONBOARD-COMPLETE-FLAG` - Persist Global Tutorial Completion to Meta Save
> **İlerleme:** `370 / 441` tracker checkbox'ı tamamlandı - `%83,90`
...
| Tutorial | İlk Day worker ratio, affordable Basic Archer, low ammo, Castle Heart, regular Council, Day repair ve ilk Night ability-key adımları non-modal pulse, English yönlendirme ve durable meta flag ile aktif; accepted player action prompt'tan önce gelirse completion kaybolmuyor, controller transaction/assignment/modal açma çağrısı yapmıyor, blocking pause sırasında cue zinciri kurmuyor ve yedi cue exact ekonomi/worker snapshot regresyonuyla kilitli | `[~]` Package I ilk yedi adım + preemptive completion, global transaction-free ve no-modal-chain invariant'ları tamamlandı |
