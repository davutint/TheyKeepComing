...
- [x] Aynı explicit Night karesinde 1.000 canonical Basic Archer + 10.000 enemy + 1.000 aktif gameplay projectile birleşik görsel kabulü tamamlandı. Production `NewGameScene` 1920x1080 capture'ında bounded presentation stride `21` ile `48` projectile görünür kaldı; gerçek Night overlay alpha `0,500`, global light `0,680` ve phase telemetry `night` olarak doğrulandı. Canonical `HordeScalePlayModeTests` targeted `1/1` geçti; capture `DW_V1_10K_1K_NIGHT_VISUAL_ACCEPTANCE.png` adıyla reproducible üretiliyor.
